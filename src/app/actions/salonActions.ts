...
import { db, auth as firebaseAuth } from '@/lib/firebase'; // Import auth to get current user
import { 
  collection, 
  getDocs, 
  addDoc, 
  ...
  deleteDoc,
  query,
  // where, // Keep for potential future use
...
// Ensure input type for addSalon matches the schema minus 'id' but includes 'ownerUid'
...
export async function addSalon(data: AddSalonInput): Promise<{ success: boolean; salon?: Salon; error?: string }> {
  ...
  if (!data.ownerUid) { // ownerUid is now mandatory
  ...
  // Use SalonSchema.omit({id: true}) for validation because `id` is auto-generated by Firestore
...
export async function updateSalon(data: Salon): Promise<{ success: boolean; salon?: Salon; error?: string }> {
  ...
  if (!data.ownerUid) { // ownerUid should be present for updates as well
      return { success: false, error: "Owner UID is missing in salon data for update." };
  }

  const validationResult = SalonSchema.safeParse(data); // Validates the full Salon object including id and ownerUid
  if (!validationResult.success) {
    const errorMessages = JSON.stringify(validationResult.error.flatten().fieldErrors);
  ...
  try {
    ...
    const { id, ...dataToUpdate } = validationResult.data; // Exclude id from the object passed to updateDoc
...
export async function deleteSalon(id: string): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    // Future check: Ensure only the owner can delete, using Firebase rules or backend logic
    const salonDocRef = doc(db, SALONS_COLLECTION, id);
